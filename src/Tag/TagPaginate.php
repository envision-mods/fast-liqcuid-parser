...
class TagPaginate extends AbstractBlock
{
	...
	public function __construct($i, $n, $markup, array &$tokens, FileSystem $fileSystem = null)
	{
		...
		$syntax = preg_match('/(' . Liquid::$config['VARIABLE_NAME'] . ')\s+by\s+(\w+)/', $markup, $matches);

		if ($syntax) {
			$this->collectionName = $matches[1];
			$this->numberItems = $matches[2];
	...
	public function render(Context $context)
	{
		...
		$this->currentPage = intval($context->get(Liquid::$config['PAGINATION_CONTEXT_KEY']));
		...
		$pageRequestKey = Liquid::$config['PAGINATION_REQUEST_KEY'];
