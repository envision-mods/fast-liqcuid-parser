...
class TagInclude extends AbstractTag
{
	...
	{
		$regex = preg_match('/(["\'])?(?|((?:(?!\1).)+)\1|([a-zA-Z_0-9.-]+))(?:\s+(with|for))?(?(3)\s+(' . Liquid::get('QUOTED_FRAGMENT') . '+))/', $markup, $matches, PREG_UNMATCHED_AS_NULL);
		...
		$this->collection = $matches[3] == "for";
		$this->variable = $matches[4];
	...
	public function parse(int $i, int $n, array &$tokens)
	{
		...
		if (!$cache) {
			...
			$this->document = new Document(0, count($templateTokens), '', $templateTokens, $this->fileSystem);
			return;
		}
