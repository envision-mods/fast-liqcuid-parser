...
class Template
{
	public const CLASS_PREFIX = '\Liquid\Cache\\';
	...
	public static function tokenize($source)
	{
		return empty($source)
			? array()
			: preg_split('/({[{%].*?[%}]})/s', $source, -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY);
	}
	...
	private function parseAlways($source)
	{
		...
		$this->root = new Document(0, count($tokens), '', $tokens, $this->fileSystem);
